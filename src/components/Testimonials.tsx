      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {testimonials.map((t, i) => (
            <article
              key={i}
              className="relative flex flex-col rounded-xl border border-white/10 bg-black/45 backdrop-blur-sm p-6 pt-8 shadow-lg shadow-black/20"
            >
              <Quote
                className="absolute top-4 right-4 h-8 w-8 text-accent/90"
                aria-hidden
                strokeWidth={1.25}
              />
              <blockquote className="text-white/85 text-sm leading-relaxed mb-6 flex-1 pr-6">&ldquo;{t.quote}&rdquo;</blockquote>
